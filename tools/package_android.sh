...
if [ ! -d builddir ]; then
  ...
  meson builddir -Dplatform=android -Dopenblas-num-threads=0 -Denable-tflite-interpreter=false -Denable-tflite-backbone=false -Denable-fp16=true -Domp-num-threads=4 -Dhgemm-experimental-kernel=false -Denable-ggml=true ${filtered_args[@]}
else
  ...
  pushd builddir
    ...
    #enable-tflite-interpreter=false is just temporally until ci system is stabel  
    #enable-opencl=true will compile OpenCL related changes or remove this option to exclude OpenCL compilations.
    meson configure -Dplatform=android -Dopenblas-num-threads=0 -Denable-tflite-interpreter=false -Denable-tflite-backbone=false -Denable-fp16=true -Domp-num-threads=4 -Dhgemm-experimental-kernel=false -Denable-ggml=true ${filtered_args[@]}
    meson --wipe
  popd
fi
