...
void FsuWeightPool::weightFileOpen() {
  if (fd > 0)
    return;
  fd = open(weight_file_path.c_str(), O_RDWR | O_CREAT | O_DIRECT, 0666UL);
...
void FsuWeightPool::validate(unsigned int id) {
  ...
  char *ptr = static_cast<char *>(
    mmap(nullptr, len, PROT_READ | O_DIRECT, MAP_PRIVATE | MAP_POPULATE, fd, off));
  if (ptr == MAP_FAILED) {
  ...
  void *now_ptr = static_cast<void *>(ptr + diff);

  elements[id].memory_ptr = now_ptr;
  elements[id].mem_data->setAddr((void *)now_ptr);
  elements[id].mem_data->setValid(true);
  elements[id].active = true;
  elements[id].load_state = LoadState::Loaded;
  elements[id].mmap_ptr = ptr;
  elements[id].mmap_len = len;
}
...
void FsuWeightPool::inActive(unsigned int order) {
  ...
  for (auto &id : exec_ids) {
    ...
    // auto ret = munmap(elements[id].mmap_ptr, elements[id].mmap_len);
  }
}
...
bool FsuWeightPool::checkAllLoadComplete(unsigned int order) {
  ...
  for (auto &id : exec_ids) {
    madvise(elements[id].mmap_ptr, elements[id].mmap_len, MADV_POPULATE_READ);
  }
  return true;
}
