...
public:
  MockCachePoolFSU(const std::string &file_path, const std::string &name,
    ...
    nntrainer::CachePool(file_path, name,
                         exec_mode) {
...
void MakeWightFile(size_t size) {
  std::ofstream outFile("weight.bin", std::ios::binary);
...
TEST_F(CachePoolFSUTest, validate_01_p) {
  MakeWightFile(4096);
  EXPECT_CALL(*pool, validate).Times(1);
  EXPECT_CALL(*pool, invalidate).Times(testing::AtLeast(1));

  std::shared_ptr<nntrainer::MemoryData> mem;
  pool->setWeightOffset({std::make_pair(0, 4096)});
  auto idx = pool->requestMemory(4096, 4, 5);
