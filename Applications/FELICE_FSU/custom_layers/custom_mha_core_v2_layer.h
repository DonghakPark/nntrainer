...
public:
   ...
   */
  WIN_EXPORT CustomMHACoreV2Layer(CustomMHACoreV2Layer &&rhs) noexcept = default;

  /**
   * @brief  Move assignment operator.
   * @parma[in] rhs CustomMultiHeadAttentionLayer to be moved.
   */
  WIN_EXPORT CustomMHACoreV2Layer &operator=(CustomMHACoreV2Layer &&rhs) = default;
...
private:
  ...
  void compute_kcaches(const float *in, const uint16_t *kcache,
                       float *output, int seq, int num_cache_head,
                       int group_size, int head_dim, bool process_all,
                       BS::thread_pool<> &pool);
