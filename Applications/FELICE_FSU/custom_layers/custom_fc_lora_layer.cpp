...
void FullyConnectedLayer::setLora(nntrainer::RunLayerContext &context, const std::string file_path) {
  nntrainer::Tensor &weight = context.getWeight(weight_idx[FCParams::weight]);

  std::cout << context.getName() << " lora layer read end" << std::endl;
}

void FullyConnectedLayer::updateTensorsByInputDimensions(nntrainer::RunLayerContext &context,
