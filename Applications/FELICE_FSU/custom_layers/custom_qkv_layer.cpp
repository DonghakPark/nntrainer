...
void QKVLayer::finalize(nntrainer::InitLayerContext &context) {
  ...
  output_dims[1].width(unit/12);
  
  output_dims[2].width(unit/12);
...
void QKVLayer::forwarding(nntrainer::RunLayerContext &context,
                                     bool training) {
  return;
}

void QKVLayer::incremental_forwarding(
  ...
  nntrainer::Tensor &Qhidden_ = context.getOutput(0);
  nntrainer::Tensor &Khidden_ = context.getOutput(1);
  nntrainer::Tensor &Vhidden_ = context.getOutput(2);
  ...
  futures.emplace_back(pool.submit_task([=](){
   ...
   for(auto &fut : futures)
    fut.get();
  //std::cout << Qhidden_.getData<float>() << " " <<Khidden_.getData<float>() << " "<<Vhidden_.getData<float>() << std::endl;
