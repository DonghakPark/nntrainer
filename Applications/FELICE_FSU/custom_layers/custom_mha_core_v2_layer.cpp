...
void CustomMHACoreV2Layer::compute_kcaches(const float *in, const uint16_t *kcache,
  ...
  for (int i = seq_end - 1; i >= seq_start; --i) {
    ...
    size_t start_row = local_window_size < num_rows ? num_rows - local_window_size : 0;
    ...
    for (int n = 0; n < num_cache_head; ++n) {
      ...
      for (int row = start_row; row < num_rows; ++row) {
        ...
        for (int g = 0; g < gqa_size; ++g) {
          const float *a_ptr = in + num_cache_head * gqa_size * head_dim * i + n * gqa_size * head_dim + g * head_dim;
          const float *b_row;
          ...
          output[out_offset + (local_window_size == UINT_MAX || num_rows < local_window_size ? row : row - (num_rows - local_window_size)) * num_cache_head * gqa_size + n * gqa_size + g] =
...
void CustomMHACoreV2Layer::one_batch_incremental_forwarding(
  ...
  // This thread_pool should be from nntrainer::Tensor for latency
  ...
  nntrainer::Tensor out_(1, 1,
                         (from) ? to : calc_attn_index(to),
                         num_heads_Q, b_projected_query_step.getTensorType());

  unsigned int gqa_size = num_heads_Q / num_heads_KV;

  compute_kcaches(b_projected_query_step.getData<float>(), b_cached_key.getData<uint16_t>(), out_.getData<float>(), to,
                  num_heads_KV, gqa_size, head_dim, (from) ? false : true, pool);

  // auto end_kcache =
  //   std::chrono::high_resolution_clock::now(); // log the start_prefill time

  // compute_kcaches includes below computation
  // out_.multiply_i(1 / sqrt((float)head_dim));
  ...
  //     std::chrono::duration_cast<std::chrono::nanoseconds>(end_prepare -
  //                                                           start_prepare);
  // auto kcache_duration =
  //     std::chrono::duration_cast<std::chrono::nanoseconds>(end_kcache -
  //                                                           start_kcache);
  // auto softmax_duration =
  //     std::chrono::duration_cast<std::chrono::nanoseconds>(end_softmax -
  //                                                           start_softmax);
  // auto vcache_duration =
  //     std::chrono::duration_cast<std::chrono::nanoseconds>(end_vcache -
  //                                                           end_softmax);
  // std::cout <<to << " -- P: "<<prepare_duration.count() << " K:
  // "<<kcache_duration.count() << " S: "<< softmax_duration.count()<< " V: "<<
  // vcache_duration.count()<<std::endl;
}
...
void CustomMHACoreV2Layer::softmax_row_AVX2_inplace(float *qk_out,
                                                    size_t start_row,
                                                    size_t end_row,
                                                    size_t num_heads) {
...
void CustomMHACoreV2Layer::softmax_row_AVX2(float *qk_out, size_t start_row,
...
void CustomMHACoreV2Layer::softmax_triangle_AVX2(nntrainer::Tensor &qk_out,
  ...
  if (from) {
    size_t start_row = 0;
    size_t end_row = local_window_size == UINT_MAX ? from + 1 : local_window_size;
...
void CustomMHACoreV2Layer::compute_fp16vcache_fp32_transposed(
  ...
  for (int i = seq_end - 1; i >= seq_start; --i) {
    ...
    for (int n = 0; n < num_cache_head; ++n) {
      ...
      for (int j = i < local_window_size ? 0 : i + 1 - local_window_size; j <= i; ++j) {
        if (j + 1 < seq) {
          const uint16_t *next_vptr =
            vcache + ((j + 1) * num_cache_head + n) * head_dim;
          _mm_prefetch(reinterpret_cast<const char *>(next_vptr),
                        _MM_HINT_T0);
        }
        ...
        for (int h = 0; h < gqa_size; ++h) {
          /*             float a_val =
              ...
              n];
            */
          float a_val = in[a_row_start + ((local_window_size == UINT_MAX || i < local_window_size ? j : j - (i + 1 - local_window_size)) * gqa_size * num_cache_head +
                                          n * gqa_size + h)];
  ...
  /*
       for(int i=0;i<seq;++i){
         int a_row_start=(i*(i+1))/2*num_cache_head*gqa_size;
         ...
         for(int n=0;n<num_cache_head;++n){
          for(int h=0;h<gqa_size;++h){
             for(int d=0;d<head_dim;++d){
              float sum=0.0;
              for(int j=0;j<=i;++j){
                  int a_idx = a_row_start +(j*gqa_size+h)*num_cache_head+n;
                   int v_idx = j*num_cache_head*head_dim + n*head_dim+d;
                   sum += in[a_idx]*convert_scalar(vcache[v_idx]);
                 }
               output[i*num_cache_head*gqa_size*head_dim+n*gqa_size*head_dim+h*head_dim
      + d]= sum;
             }
          }
         }
...
size_t CustomMHACoreV2Layer::calc_attn_index(size_t i) {
  ...
  if (local_window_size == UINT_MAX || i < local_window_size) {
    ...
    return (local_window_size * (local_window_size + 1)) / 2 + (i - local_window_size) * local_window_size;
  }
};
...
void CustomMHACoreV2Layer::updateTensorsByInputDimensions(nntrainer::RunLayerContext &context,
  ...
  //   context.updateTensor(weight_idx[AttentionParams::dropout_mask], dropout_mask_dim);
  // }
}
