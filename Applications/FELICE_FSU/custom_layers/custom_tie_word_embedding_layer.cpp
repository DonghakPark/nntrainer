...
void CustomTieWordEmbeddingLayer::incremental_forwarding_lmhead(
  ...
  for (unsigned int b = 0; b < b_size; ++b) {
    ...
    if (use_vocab_selection) {
      ...
      if (weight.getDataType() == nntrainer::TensorDim::DataType::Q6_K) {
        // Q6_K weight is already supported transposed dot internally.
...
void CustomTieWordEmbeddingLayer::updateTensorsByInputDimensions(nntrainer::RunLayerContext &context,
