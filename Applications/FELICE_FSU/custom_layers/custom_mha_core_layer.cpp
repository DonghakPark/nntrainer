...
void CustomMHACoreLayer::updateTensorsByInputDimensions(nntrainer::RunLayerContext &context,
  ...
  ml::train::TensorDim attention_weight_dim = context.getTensor(weight_idx[AttentionParams::attention_weight]).getDim();
  ...
  unsigned int height_axis = context.getTensor(weight_idx[AttentionParams::cache_key]).getFormat() == nntrainer::Tformat::NCHW ? 2 : 1;
  unsigned int width_axis = context.getTensor(weight_idx[AttentionParams::cache_key]).getFormat() == nntrainer::Tformat::NCHW ? 3 : 2;
  ...
  context.updateTensor(weight_idx[AttentionParams::projected_key], kv_cache_dim);
  context.updateTensor(weight_idx[AttentionParams::projected_value], kv_cache_dim);
  context.updateTensor(weight_idx[AttentionParams::attention_weight], attention_weight_dim);
  context.updateTensor(weight_idx[AttentionParams::attention_output], kv_cache_dim);

  // const float dropout_rate =
  //   std::get<nntrainer::props::DropOutRate>(mha_core_props).get();
  // if (dropout_rate > epsilon) {
  //   ml::train::TensorDim dropout_mask_dim = input_dimensions[0];
  //   context.updateTensor(weight_idx[AttentionParams::dropout_mask], kv_cache_dim);
  // }
}
